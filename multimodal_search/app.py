...
def get_groq_client(api_key):
    """Cache Groq client — only created once, not on every Streamlit reload."""
    return Groq(api_key=api_key) if api_key else None

client = get_groq_client(DEFAULT_API_KEY)
                ...
                if st.session_state.get('template_bytes'):
                    with st.spinner("🚀 KALI ARCHITECT: Executing Structural Surgery..."):
                        ...
                        if edited_bytes:
                            ...
                            # Show download button IMMEDIATELY in chat so user sees it
                            st.download_button(
                                "📥 DOWNLOAD EDITED DOCUMENT",
                                data=edited_bytes,
                                file_name="Kali_AI_Final_Edit.docx",
                                mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                                key=f"inline_dl_{len(st.session_state.messages)}",
                    ...
                    # ── FIX: No file loaded — show clear instructions ──
                    st.warning(
                        "⚠️ **No document loaded!**\n\n"
                        "Kali detected edit instructions but has no file to edit.\n\n"
                        "**To fix this:**\n"
                        "1. Click the **➕ button** (bottom-left of chat)\n"
                        "2. Upload your `.docx` file\n"
                        "3. Then re-send your edit message\n\n"
                        "_Or paste the file path in the sidebar under_ **🛰️ SYSTEM LINK**"
                    )
